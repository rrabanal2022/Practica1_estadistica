...
Por otro lado, *sleep_hours*, *nutrition_score* y la variable *injury_count* están asociados a la nutrición, al descanso del jugador y al estado del jugador. Es decir, son variables determinantes para la recuperación y la salud de los mismos.
...
ggplot(df, aes(x = performance_index)) +
  geom_histogram(bins = 30) +
  labs(title = "Distribución del Performance Index")
...
ggplot(df, aes(x = training_hours)) +
  geom_histogram(bins = 30) +
  labs(title = "Distribución de horas de entrenamiento")

```

(Seguir con los demás)

A continuación vemos si hay outliers

```{r}

ggplot(df, aes(y = performance_index)) +
  geom_boxplot() +
  labs(title = "Boxplot del Performance Index")

```

```{r}

ggplot(df, aes(y = fatigue)) +
  geom_boxplot() +
  labs(title = "Boxplot de Fatigue")

```

(Seguir con el resto de predictores)


Ahora se realizará una exploración descriptiva bivariada. Con esto podremos ver relaciones preliminares.

```{r}

ggplot(df, aes(x = training_hours, y = performance_index)) +
  geom_point() +
